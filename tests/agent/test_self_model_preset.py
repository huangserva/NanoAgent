...
def test_from_config_external_memory_from_config_schema(tmp_path) -> None:
    from unittest.mock import patch
    ...
    with patch("nanobot.providers.factory.make_provider", return_value=fake_provider):
        loop = AgentLoop.from_config(config)
    assert loop.external_memory is not None
    assert loop.external_memory.retrieval_limit == 7
    assert loop.external_memory.packet_char_limit == 3000


def test_from_config_external_memory_kwarg_wins_over_config(tmp_path) -> None:
    from unittest.mock import patch
